...
impl InertTorRelay {
    ...
    fn try_generate_keys(keymgr: &KeyMgr) -> anyhow::Result<()> {
        ...
        // This key will also be generated by the key rotation task, so it's not completely needed
        // to do it here as well. However, it probably makes sense, in case anyone trys to use it
        // before the key rotation task runs.
        crate::tasks::key_rotation::generate_relaysign(keymgr)
            .context("Failed to get or generate the relay signing key")?;
...
impl<R: Runtime> TorRelay<R> {
    ...
    pub(crate) async fn run(self) -> anyhow::Result<void::Void> {
        ...
        task_handles.spawn({
            let runtime = self.runtime.clone();
            let keymgr = Arc::clone(&self.keymgr);
            async {
                crate::tasks::key_rotation::rotate_keys_task(runtime, keymgr)
                    .await

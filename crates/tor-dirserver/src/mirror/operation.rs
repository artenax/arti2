...
use tor_netdoc::{
    doc::{
        authcert::{AuthCertKeyIds, AuthCertSigned},
        netstatus::ConsensusFlavor,
    },
    parse2::{
        self,
        poc::netstatus::{cons, md, NdiDirectorySignature},
        NetdocParseable, NetdocSigned, ParseInput,
    },
};
...
enum FlavoredConsensusSigned {
    /// For plain consensuses.
    Ns(cons::NetworkStatusSigned),

    /// For microdescriptor consensus.
    Md(md::NetworkStatusSigned),
}
...
impl StaticEngine {
    ...
    async fn auth_certs(
        ...
        // Fire it off.
        let (resp, certs) = self
            .send_request::<_, AuthCertSigned>(endpoint, requ)
            .await?;
...
mod test {
    ...
    use tor_netdoc::parse2::NetdocSigned;
